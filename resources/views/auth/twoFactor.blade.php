                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="text-center mb-4">
                            <p>{{ __('Please enter the OTP from your authenticator app or a recovery code:') }}</p>

                            @if (session('status'))
                                <div class="alert alert-success">
                                    {{ session('status') }}
                                </div>
                                <a href="{{ route('dashboard') }}" class="btn btn-hero btn-primary">{{ __('Continue') }}</a>
                            @else
                                <form action="{{ route('2fa.verify') }}" method="POST">
                                    @csrf
                                    <div class="mb-4">
                                        <div class="input-group input-group-lg">
                                            <input type="text" name="one_time_password" class="form-control @error('one_time_password') is-invalid @enderror" id="one_time_password" placeholder="{{ __('Enter OTP or recovery code') }}" required>
