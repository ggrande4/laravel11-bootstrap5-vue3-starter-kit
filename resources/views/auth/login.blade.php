                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @if(session()->has('session_expired'))
                            <div class="alert alert-danger text-danger session-expired">{{ session()->get('session_expired') }}</div>
                        @endif
                        ...
                        <form action="{{ route('login') }}" method="POST">
                            ...
                            <div class="text-center mb-4">
                                <p>{{ __('Or') }}</p>
                                <div class="d-flex flex-column align-items-stretch col-8 mx-auto">
                                    <a href="{{ route('social.login', 'google') }}" class="btn btn-outline-secondary btn-lg mb-2">
                                        <i class="me-3"><img class="h-1 mb-1" src="/assets/media/brands/google-icon.svg" /></i>{{ __('Sign In with Google') }}
                                    </a>
                                    <a href="{{ route('social.login', 'facebook') }}" class="btn btn-primary btn-lg mb-2">
                                        <i class="fab fa-facebook-f opacity-50 me-2"></i>{{ __('Sign In with Facebook') }}
                                    </a>
                                </div>
                            </div>
