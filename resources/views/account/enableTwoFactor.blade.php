<x-auth-layout>
    <div class="bg-image" style="background-image: url({{ asset('assets/media/images/image1.webp') }});">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="text-center mb-4">
                            <p>{{ __('Scan the QR code below with your authenticator app:') }}</p>
                            <div>{!! $QR_Image !!}</div>

                            <p class="mt-4">{{ __('Save these recovery codes in a safe place. You can use them to access your account if you lose access to your authentication app. Each code can be used only once.') }}</p>
                            ...
                            <form action="{{ route('account.verifyTwoFactor') }}" method="POST">
                                @csrf
                                <div class="mb-4">
                                    <div class="input-group input-group-lg">
                                        <input type="text" name="one_time_password" class="form-control @error('one_time_password') is-invalid @enderror" id="one_time_password" placeholder="{{ __('Enter the OTP from your authenticator app') }}" required>
                                        <x-generic.input-error :messages="$errors->get('one_time_password')" class="invalid-feedback mt-2" />
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-hero btn-primary">{{ __('Verify') }}</button>
                            </form>
                        </div>
                    </div>
                </div>
