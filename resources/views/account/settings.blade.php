            ...
            <div class="col-md-3 mt-5">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="text-center mb-4">
                            ...
                            @if($user->two_factor_enabled)
                                ...
                                <form action="{{ route('account.disableTwoFactor') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-hero btn-danger">{{ __('Disable Two-factor Authentication') }}</button>
                                </form>
                            @else
                                <p>{{ __('Two-factor authentication is not enabled.') }}</p>
                                <form action="{{ route('account.enableTwoFactor') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-hero btn-primary">{{ __('Enable Two-factor Authentication') }}</button>
                                </form>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="card-footer text-end">
                        <a href="{{ url('/backend') }}" class="fw-medium">{{ __('Close') }}</a>
                    </div>
                </div>
            </div>
        </div>
